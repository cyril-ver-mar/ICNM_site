(() => {
  ...
  function isBviOn() {
    ...
    return !!(
      ...
      (root && (root.classList.contains("bvi-active") || root.classList.contains("is-bvi")))
    );
  }

  if (reduceMotion || isBviOn()) return;
...
function createLattice(host, canvas, initial) {
  ...
  function entriesKeepout() {
    const pills = host.querySelectorAll(".ichnm-hero-pills .ichnm-pill, .hero-entries .hero-pill");
  ...
  function bootLattices() {
    document.querySelectorAll(".ichnm-hero, .hero, .lattice-stage").forEach((host) => {
      const canvas = host.querySelector(".hero-lattice");
      if (!canvas || !(canvas instanceof HTMLCanvasElement)) return;
      createLattice(host, canvas, loadLatticeConfig());
    });
  }
  bootLattices();
})();
